...
>(({ className, sideOffset = 4, dark, iconHelpTooltip, ...props }, ref) => (
  <TooltipPrimitive.Content
    ref={ref}
    sideOffset={sideOffset}
    className={cn(
      'bg-gray-500 body-xs text-white animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-50 overflow-hidden rounded p-3',
      dark && 'bg-surface-inverted text-text-inverted',
      className
    )}
    {...props}
  >
    <p className='body-sm font-semibold'>{iconHelpTooltip.title}</p>
    <Spacer size={1} />
    <p className='body-sm'>{iconHelpTooltip.description}</p>
  </TooltipPrimitive.Content>
));
